...
### Причина
Система деплоя пытается запустить `python3 /app/app.py`, но указывает на неверный путь или файл отсутствует.

### Решение

В данном репозитории **app.py уже существует** и является правильной точкой входа для FastAPI приложения.

#### Для Amvera
Убедитесь, что в `amvera.yml` установлено:
```yaml
run:
...
Файл `server.py` запускает uvicorn с приложением из `app.py`.
